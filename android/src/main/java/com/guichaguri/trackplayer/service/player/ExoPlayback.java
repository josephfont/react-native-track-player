...
public abstract class ExoPlayback<T extends Player>
  ...
  public void updateTrack(int index, Track track) {
    ...
    if (currentIndex == index) manager.getMetadata().updateMetadata(track);
  }
  ...
  public void setRate(float rate) {
    player.setPlaybackParameters(
      new PlaybackParameters(rate, player.getPlaybackParameters().pitch)
    );
  }
  ...
  public void onTimelineChanged(
    ...
    if (
      (
        reason == Player.TIMELINE_CHANGE_REASON_PREPARED ||
  ...
  public void onPositionDiscontinuity(int reason) {
    ...
    if (lastKnownWindow != player.getCurrentWindowIndex()) {
      Track previous = lastKnownWindow == C.INDEX_UNSET ||
        lastKnownWindow >= queue.size()
        ? null
        : queue.get(lastKnownWindow);
      ...
      if (
        ...
        if (
          lastKnownWindow >= player.getCurrentTimeline().getWindowCount()
        ) return;
        long duration = player
          .getCurrentTimeline()
          .getWindow(lastKnownWindow, new Window())
          .getDurationMs();
  ...
  private void handleId3Metadata(Metadata metadata) {
    String title = null, url = null, artist = null, album = null, date =
      null, genre = null;
